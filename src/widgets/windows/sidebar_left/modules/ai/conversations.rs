...
pub struct ConversationItem {
    pub conversation: SqlAiConversation,
    pub root: gtk4::Box,
...
impl ConversationItem {
    pub fn new(conversation: SqlAiConversation) -> Self {
        ...
        let title_label = gtk4::Label::new(Some(&conversation.title));
                    ...
                    while let Ok(message) = receiver.recv().await {
                        match message {
                            ...
                            openai::AIChannelMessage::ConversationDeleted(conversation_id) => {
                                ...
                                if let Some(pos) = conversations.iter().position(|item| item.conversation.id == conversation_id) {
                            ...
                            openai::AIChannelMessage::CycleFinished => {
                                ...
                                let conversations = me.conversations.borrow();
                                for item in conversations.iter() {
                                    if item.conversation.id == current_conversation.id {
